/**
 * dosirak.store vendor-draft first-party connector
...
const PLATFORM_ID = "dosirak.store";
const DEFAULT_DRAFT_PATH = "/api/vendor-draft";
const FETCH_TIMEOUT_MS = 12000;

function connectorConfig() {
  const url = String(process.env.DOSIRAK_VENDOR_DRAFT_URL || "").trim();
  const secret = String(process.env.DOSIRAK_AFFILIATE_CONNECTOR_SECRET || "").trim();
  return {
    url,
    secret,
...
function buildMenuCatalogSnapshot() {
  ...
  const prices = Object.values(MENUS).map((row) => Number(row.unitPrice || 0)).filter((n) => n > 0);
  const min = prices.length ? Math.min(...prices) : 0;
  const max = prices.length ? Math.max(...prices) : 0;
  return {
    menu_catalog_ids: ids.slice(0, 50),
    menu: [...menuLines, ...setLines].join(" / ").slice(0, 2000),
    price_range: min && max ? `${min.toLocaleString("ko-KR")}~${max.toLocaleString("ko-KR")}원` : "",
  };
}
...
/**
 * @param {object} input
 * @param {object} input.consents — { privacyAt, termsAt } ISO strings required
 * @param {object} [input.merchant] — overrides (phone required for live/dry submit)
 * @param {boolean} [input.dryRun]
 */
function buildVendorDraftPayload(input = {}) {
  ...
  // never autofill bank / secrets
  const data = {
    source_template: "jjajangnara",
    source_instance: String(profile.source || "jjajangnara-template").slice(0, 80),
    biz_name: String(profile.storeName || "짜장나라").slice(0, 120),
    rep_name: String(profile.repName || "").slice(0, 80),
    phone,
    email: String(profile.email || "").slice(0, 120),
    region: String(profile.addressHint || "").slice(0, 200),
    areas: String(profile.deliveryNotes || profile.addressHint || "").slice(0, 300),
    menu: catalog.menu,
    price_range: catalog.price_range,
    menu_catalog_ids: catalog.menu_catalog_ids,
    memo: String(input.memo || "arkaon affiliate_draft_v1").slice(0, 500),
...
async function postVendorDraft(input = {}) {
  const cfg = connectorConfig();
  let payload;
  try {
    payload = buildVendorDraftPayload(input);
  } catch (error) {
    return {
      ok: false,
  ...
  if (!cfg.configured) {
    return {
      ...
      message: "DOSIRAK_VENDOR_DRAFT_URL / DOSIRAK_AFFILIATE_CONNECTOR_SECRET 미설정",
      resumeHint: "Netlify env 설정 후 onboarding-execute 재시도",
      draftPreview: {
        biz_name: payload.data.biz_name,
  ...
  try {
    ...
    try {
      ...
      body = { ok: false, error: "invalid_json_from_dosirak", raw: text.slice(0, 200) };
    }
    return {
      ok: Boolean(body.ok),
    ...
    return {
      ok: false,
      code: timeout ? "DOSIRAK_TIMEOUT" : "DOSIRAK_FETCH_FAILED",
